...
const QUICK_ACTIONS = [
  { key: "map", label: "Mapa", href: "/map", icon: Map },
  { key: "register", label: "Registro", href: "/trees/new", icon: PlusCircle },
  { key: "management", label: "Gerenciamento", href: "/management", icon: Trees },
  { key: "users", label: "Usuarios", href: "/users", icon: Users },
  { key: "records", label: "Registros", href: "/records", icon: ClipboardList },
  { key: "profile", label: "Perfil", href: "/admin/profile", icon: UserCircle },
];
...
export default function AdminDashboardHome() {
  return (
    <DashboardRoleLayout
      defaultUserName="Dra. Helena Cavalcanti"
      headerTitle="Visao geral"
      headerSubtitle="42 arvores monitoradas"
      role={UserRole.ADMIN}
    >
      ...
      <section className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <DashboardCard className="bg-white/55 shadow-none">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button text="Ver fila" icon={ListChecks} href="/approvals" variant="ghost" />
          </div>
        </DashboardCard>
      ...
      <section className="mt-6 flex flex-wrap gap-3">
        <Button text="Abrir mapa" icon={Map} href="/map" variant="primary" />
        <Button text="Registrar arvore" icon={PlusCircle} href="/trees/new" variant="outline" />
      </section>
    </DashboardRoleLayout>
  );
}
